...
### Fixed
- 확대/축소 시 보던 페이지를 벗어나던 문제 — 전체 페이지를 다시 만들지 않고 기존 슬롯 크기만 바꾸며, 화면 가운데 보던 위치(페이지 + 페이지 안 위치)를 그대로 유지
- 확대/축소 중에는 이전 이미지를 늘려 보여주고 선명한 이미지로 교체 (깜빡임/로드중 표시 없음)
- 크기가 다른 페이지(표지 세로 + 펼침 가로)가 뷰포트 위쪽에서 렌더되며 크기가 바뀌어도 보던 위치가 밀리지 않도록 보정

## [0.2.21] - 2026-09-26

### Fixed
- 리버스 프록시(시놀로지) 환경에서 PDF가 열리지 않던 문제 — 호스트가 `preview_url`을 내부 포트(`https://host:8482/api/...`)로 만들어 페이지(`https://host`)와 출처가 달라 `fetch`/pdf.js가 실패하고 브라우저 기본 뷰어 iframe으로 떨어졌음. 같은 호스트명이면 현재 페이지 origin 기준 경로로 바꿔 요청

## [0.2.20] - 2026-09-26

### Fixed
- 파일 목록에서 2번째 이후 PDF를 선택해도 **첫 파일이 계속 보이던 문제** — 첫 파일 다운로드가 끝나기 전에 다른 파일을 누르면, 늦게 끝난 첫 파일 로드가 선택한 파일을 덮어쓰던 경쟁 조건 수정 (마지막으로 선택한 파일만 표시)
- 스크립트 캐시 버스터 `share-pdf.js?v=0.2.20` — 브라우저가 이전 JS를 계속 쓰지 않도록
- 파일 전환 시 이전 파일 페이지/썸네일을 즉시 비우고 **불러오는 중** 표시, 이전 PDF 문서는 `destroy()`로 해제

## [0.2.19] - 2026-09-22

### Fixed
- 왼쪽 페이지 썸네일은 초기 로드 시 **처음 6개만** 렌더하고, 나머지는 **페이지 로드중** 표시 — 레일 스크롤(또는 해당 페이지 이동) 시 추가 로드

## [0.2.18] - 2026-09-22

### Fixed
- 본문 페이지 fit 너비를 더 줄여 왼쪽 썸네일 레일(96px + gap)과 겹치지 않도록 조정 — 스크롤러 `padding-left`로 레일 영역을 비우고 그 안쪽에 맞춤

## [0.2.17] - 2026-09-22

### Fixed
- 초기 로드 시 본문 페이지가 스테이지(빨간 박스) 높이/너비에 맞게 **fit-to-viewport** 되도록 조정 (과도한 overflow 제거)
- 초기 로드에서 전체 페이지를 한꺼번에 `getPage`/`render` 하지 않음 — 메인 스크롤러·왼쪽 썸네일 레일의 **보이는 구간만** 먼저 로드
- 아직 렌더되지 않은 슬롯에는 **페이지 로드중** 표시, 스크롤/페이지 이동 시 추가 로드
